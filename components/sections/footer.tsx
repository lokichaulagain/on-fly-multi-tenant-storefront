...
export default function Footer() {
  ...
  const socialLinks = [
    {
      title: "Facebook",
      icon: <Facebook size={16} className=" text-blue-700" />,
    ...
    {
      title: "WhatsApp",
      icon: <IconWhatsApp size={16} className="text-green-500" />,
    ...
    {
      title: "TikTok",
      icon: <IconTiktok size={16} className="text-purple-500" />,
  ...
  return (
    <footer className="mt-16">
        ...
        <div className="relative z-10 container mx-auto px-4 md:px-24 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className=" col-span-4 sm:col-span-4 lg:col-span-1">
            <div className="bg-white/5 p-3 inline-block rounded-lg  shadow-inner">
              <Image src={store?.store_logo || "/placeholder.svg"} alt="logo" width={32} height={32} />
            </div>
            ...
            <div className=" space-y-2 text-xs font-light ">
              {quickLinks.map((link, index) => (
                ...
                >
                  ...
                  <ChevronRight size={16} className=" mt-1 group-hover:translate-x-1 duration-300" />
                </Link>
              ))}
            </div>
          </div>
      ...
      <p className=" flex items-center justify-center gap-1 text-[10px]  bg-blue-500 text-white py-1">
        <Zap size={14} />
        Powered by
        <Link target="_blank" href="https://fenzora.com" className=" font-medium underline ">
          Fenzora
        </Link>
      </p>
    </footer>
...
function IconTiktok({ size = 20, className }: { size?: number; className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} className={className} viewBox="0 0 48 48">
      <path
        fill="none"
        stroke="currentColor"
